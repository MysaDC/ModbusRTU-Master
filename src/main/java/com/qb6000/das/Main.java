...
import com.qb6000.das.service.PollingRuntime;
...
public final class Main {
    ...
    private static void run(String[] args) throws Exception {
        ...
        int controllerCount = config.controllers().size();

        ObjectWriter telemetryWriter = builder()
            .addModule(new JavaTimeModule())
            .disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS)
            .build()
            .writerFor(TelemetryMessage.class);

        MqttPublisher mqttPublisher = new MqttPublisher(config.mqtt());
        PollingRuntime pollingRuntime = new PollingRuntime(controllerCount);
        ...
        List<ModbusReader> modbusReaders = new ArrayList<>(controllerCount);
        List<PollingService> pollingServices = new ArrayList<>(controllerCount);
        ...
        log.info("QB6000 采集服务已启动，控制器数量：{}，配置文件：{}",
            controllerCount, configPath.toAbsolutePath());
